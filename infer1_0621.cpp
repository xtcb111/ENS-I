...
double variance(deque<double>temp_1, deque<double>temp_2) {//get the covariance, variance
...
double mysum(deque<double>iter) {
    double sum = 0;
    for (int i = 0; i < iter.size(); i++)
        sum += iter[i];
    return sum;
}
deque<double> Simulation_sum(deque<deque<double>>Sample, int sample_num, int N, int state) {
    deque<double>state_sum;
    for (int i = 0; i < sample_num; i++) {
        deque<double>prob_i;
        for (int j = 0; j < N; j++) {
            prob_i.push_back(Sample[i][j + state * N]);
        }
        state_sum.push_back(mysum(prob_i));
        prob_i.clear();
    }
    return state_sum;
}
